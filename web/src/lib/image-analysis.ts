...
export function detectHaloArtifacts(
  ...
  const scanRadius = faceData
    ? Math.max(faceData.w, faceData.h) * 0.8
    ...
    for (let r = scanRadius * 0.7; r < scanRadius * 1.3; r += 3) {
      ...
      if (x >= 1 && x < width - 1 && y >= 1 && y < height - 1) {
        ...
        if (brightness > 220 && brightness < 252) {
          ...
          const innerIdx =
            ((y - Math.sign(Math.sin(angle))) * width +
              (x - Math.sign(Math.cos(angle)))) *
            4;
          const innerBrightness =
            (data[innerIdx] + data[innerIdx + 1] + data[innerIdx + 2]) / 3;
  ...
  const roughnessRatio =
    edgePixelCount > 0 ? transitionScore / edgePixelCount : 0;
...
export function analyzeFaceLighting(
  ...
  const leftAvg =
    leftBrightness.reduce((a, b) => a + b, 0) / leftBrightness.length;
  const rightAvg =
    rightBrightness.reduce((a, b) => a + b, 0) / rightBrightness.length;

  // Calculate asymmetry as percentage difference
  const avgBrightness = (leftAvg + rightAvg) / 2;
  const asymmetry =
    (Math.abs(leftAvg - rightAvg) / Math.max(avgBrightness, 1)) * 100;
...
export function analyzeImage(
  ...
  const eyeTilt = faceData
    ? calculateEyeTilt(faceData.leftEye, faceData.rightEye)
    : 0;
