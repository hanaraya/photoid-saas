 ...
 */
async function selectCountryStandard(
...
test.describe('Country Standards - Photo Dimensions', () => {
  ...
  for (const standard of COUNTRY_STANDARDS) {
    test(`${standard.name} (${standard.id}) - produces correct dimensions (${standard.specW}×${standard.specH}${standard.unit})`, async ({
      page,
    }) => {
...
test.describe('Country Standards - Compliance Checks', () => {
  ...
  test('US standard shows correct compliance requirements', async ({
    page,
  }) => {
...
test.describe('Country Standards - Sheet Layout', () => {
  ...
  test('4x6 sheet contains multiple photos for US standard', async ({
    page,
  }) => {
    await page.locator('input[type="file"]').setInputFiles(TEST_IMAGE);
  ...
  test('Sheet for UK standard contains appropriately sized photos', async ({
    page,
  }) => {
    await selectCountryStandard(page, 'uk');
...
test.describe('Country Standards - Standard Switching', () => {
  ...
  test('Standard selection persists through photo upload', async ({ page }) => {
    ...
    await expect(page.locator('text=35×45').first()).toBeVisible({
      timeout: 5000,
    });
  });
});
