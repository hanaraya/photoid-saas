...
import { analyzeImageQuality, UK_PASSPORT_SPECS } from './helpers/visual-analyzer';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const SCREENSHOT_DIR = path.join(__dirname, '../../../test-results/screenshots/uk-passport');
...
test.describe('UK Passport Photo - Complete E2E Flow', () => {
  ...
  test('should display correct UK passport specifications', async ({ page }) => {
    // Verify the spec display - use .first() for multiple matches
    await expect(page.locator('text=35').or(page.locator('text=45')).first()).toBeVisible({ timeout: 5000 });
  ...
  test('should generate UK photo with correct 35x45mm dimensions', async ({ page }) => {
    const testPhoto = path.join(__dirname, '../../fixtures/test-portrait.jpg');
  ...
  test('should download generated UK passport photo', async ({ page }) => {
    ...
    if (downloadPath) {
      ...
      const analysis = await analyzeImageQuality(photoBuffer, UK_PASSPORT_SPECS);
...
test.describe('UK Passport Photo - Visual Quality Verification', () => {
  ...
  test('should detect head too small for UK specs', async ({ page }) => {
    const smallHeadPhoto = path.join(__dirname, '../../fixtures/quality-tests/bad-head-too-small.jpg');
    ...
    const checks = await getComplianceChecks(page);
    const headCheck = checks.find(c => c.label.toLowerCase().includes('head'));
  ...
  test('should detect head too large for UK specs', async ({ page }) => {
    const largeHeadPhoto = path.join(__dirname, '../../fixtures/quality-tests/bad-head-too-large.jpg');
  ...
  test('should detect shadows on face', async ({ page }) => {
    const shadowsPhoto = path.join(__dirname, '../../fixtures/quality-tests/bad-shadows.jpg');
...
test.describe('UK Passport Photo - Output Quality Analysis', () => {
  ...
  test('should produce output with acceptable background for UK', async ({ page }) => {
    const testPhoto = path.join(__dirname, '../../fixtures/test-portrait.jpg');
    ...
    const bgAnalysis = await page.evaluate(() => {
      ...
      const isLightGrey = avgR > 200 && avgG > 200 && avgB > 200 && 
    ...
    if (bgAnalysis) {
      if (!bgAnalysis.isAcceptable) {
        console.warn('GAP: UK output background is not acceptable:', bgAnalysis);
        console.warn('This may indicate background removal or image processing issues');
      }
    }
  });

  test('should produce correct aspect ratio for UK (35:45)', async ({ page }) => {
    const testPhoto = path.join(__dirname, '../../fixtures/test-portrait.jpg');
  ...
  test('should not have excessive margins in UK output', async ({ page }) => {
    ...
    const marginAnalysis = await page.evaluate(() => {
      ...
      return { topMargin, bottomMargin, marginPercent, hasExcessiveMargins: marginPercent > 20 };
    });
