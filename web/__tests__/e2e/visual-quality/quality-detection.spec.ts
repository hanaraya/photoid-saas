...
import { analyzeImageQuality, detectManipulation } from './helpers/visual-analyzer';

const FIXTURE_DIR = path.join(__dirname, '../../fixtures/quality-tests');
const SCREENSHOT_DIR = path.join(__dirname, '../../test-results/screenshots/quality-detection');
...
test.describe('Quality Detection - Exposure', () => {
  ...
  test('should detect overexposed photos', async ({ page }) => {
    ...
    const checks = await getComplianceChecks(page);
    const exposureCheck = checks.find(c => 
...
test.describe('Quality Detection - Background', () => {
  ...
  test('should detect non-white background', async ({ page }) => {
    ...
    const bgRemovalVisible = await page.locator('button:has-text("Remove Background")').isVisible({ timeout: 3000 });
    ...
    expect(bgRemovalVisible || (bgCheck && bgCheck.status !== 'pass')).toBeTruthy();
  });
...
test.describe('Quality Detection - Manipulation (CRITICAL)', () => {
  ...
  test('should detect unnatural skin tones', async ({ page }) => {
    ...
    const hasWarning = checks.some(c => c.status === 'warn' || c.status === 'fail');
  ...
  test('MANIPULATION GAP: AI-generated face detection', async ({ page }) => {
    ...
    const manipulationCheck = checks.find(c => 
  ...
  test('MANIPULATION GAP: Photoshopped background detection', async ({ page }) => {
    /**
    ...
    const bgQualityCheck = checks.find(c => 
      c.label.toLowerCase().includes('background') &&
      (c.label.toLowerCase().includes('quality') || c.label.toLowerCase().includes('artifact'))
    );
...
test.describe('Quality Detection - Resolution', () => {
  ...
  test('should detect low resolution photos', async ({ page }) => {
    ...
    console.log('Low res photo dimensions:', metadata.width, 'x', metadata.height);
...
test.describe('Quality Detection - Shadows', () => {
  ...
  test('should detect shadows on face', async ({ page }) => {
    ...
    const checks = await getComplianceChecks(page);
    const lightingCheck = checks.find(c => 
      c.label.toLowerCase().includes('light') || 
...
test.describe('Quality Detection - Comprehensive Report', () => {
  test('should generate quality report for all fixtures', async ({ page }) => {
    ...
    for (const fixture of fixtures) {
      ...
      if (localAnalysis.isBlurry && !appChecks.some(c => c.label.toLowerCase().includes('blur') && c.status !== 'pass')) {
        gaps.push('Blur not detected');
      }
      
      if (!localAnalysis.isProperlyExposed && !appChecks.some(c => 
        (c.label.toLowerCase().includes('light') || c.label.toLowerCase().includes('exposure')) && 
        c.status !== 'pass'
      )) {
        gaps.push('Exposure issue not detected');
      }
      
      if (localAnalysis.hasHaloArtifacts && !appChecks.some(c => c.label.toLowerCase().includes('halo') && c.status !== 'pass')) {
        gaps.push('Halo artifacts not detected');
      }
      
      if (manipulationAnalysis.isLikelyManipulated && !appChecks.some(c => 
        c.label.toLowerCase().includes('manipulation') && c.status !== 'pass'
      )) {
    ...
    for (const entry of report) {
      console.log(`\n${entry.fixture}:`);
      console.log('  Local issues:', entry.localIssues.length > 0 ? entry.localIssues.join(', ') : 'None');
      console.log('  App checks:', entry.appChecks.map(c => `${c.label}:${c.status}`).join(', ') || 'None');
