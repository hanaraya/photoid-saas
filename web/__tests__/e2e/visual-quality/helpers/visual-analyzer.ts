...
export async function analyzeImageQuality(
  ...
  if (!isCorrectSize) {
    issues.push(`Incorrect dimensions: ${width}x${height}, expected ${specs.width}x${specs.height}`);
  }
  ...
  for (let y = 1; y < height - 1; y += 2) {
    for (let x = 1; x < width - 1; x += 2) {
      ...
      const grayUp = channels >= 3
        ? (pixels[((y - 1) * width + x) * channels] + pixels[((y - 1) * width + x) * channels + 1] + pixels[((y - 1) * width + x) * channels + 2]) / 3
        : pixels[((y - 1) * width + x) * channels];
      
      const grayDown = channels >= 3
        ? (pixels[((y + 1) * width + x) * channels] + pixels[((y + 1) * width + x) * channels + 1] + pixels[((y + 1) * width + x) * channels + 2]) / 3
        : pixels[((y + 1) * width + x) * channels];
      
      const grayLeft = channels >= 3
        ? (pixels[(y * width + x - 1) * channels] + pixels[(y * width + x - 1) * channels + 1] + pixels[(y * width + x - 1) * channels + 2]) / 3
        : pixels[(y * width + x - 1) * channels];
      
      const grayRight = channels >= 3
        ? (pixels[(y * width + x + 1) * channels] + pixels[(y * width + x + 1) * channels + 1] + pixels[(y * width + x + 1) * channels + 2]) / 3
        : pixels[(y * width + x + 1) * channels];
  ...
  if (!isProperlyExposed) {
    if (brightness <= 80) {
      issues.push(`Image is underexposed (brightness: ${brightness.toFixed(1)})`);
    } else {
      issues.push(`Image is overexposed (brightness: ${brightness.toFixed(1)})`);
    }
  }
  ...
  for (const [x, y] of [[margin, margin], [width - margin, margin], [margin, height - margin], [width - margin, height - margin]]) {
    const idx = (y * width + x) * channels;
    if (channels >= 3) {
  ...
  const avgBgR = cornerSamples.reduce((a, s) => a + s[0], 0) / cornerSamples.length;
  const avgBgG = cornerSamples.reduce((a, s) => a + s[1], 0) / cornerSamples.length;
  const avgBgB = cornerSamples.reduce((a, s) => a + s[2], 0) / cornerSamples.length;
  
  const backgroundWhiteness = (avgBgR + avgBgG + avgBgB) / (3 * 255);
  const hasWhiteBackground = avgBgR > 240 && avgBgG > 240 && avgBgB > 240;
  
  if (!hasWhiteBackground) {
    issues.push(`Background is not white (RGB: ${avgBgR.toFixed(0)}, ${avgBgG.toFixed(0)}, ${avgBgB.toFixed(0)})`);
  }
...
export async function detectManipulation(imageBuffer: Buffer): Promise<{
  ...
  if (skinSampleCount > 0) {
    ...
    if (unnaturalPercent > 30) {
      ...
      reasons.push(`Unnatural skin tones detected (${unnaturalPercent.toFixed(1)}%)`);
    }
  }
  ...
  for (let y = 1; y < height - 1; y += 4) {
    for (let x = 1; x < width - 1; x += 4) {
      ...
      if (channels >= 3) {
        ...
        noiseVariance += Math.abs(grayCenter - grayRight) + Math.abs(grayCenter - grayDown);
        noiseCount++;
      }
    }
  }
...
export async function meetsPassportStandards(
  ...
  if (!analysis.hasWhiteBackground && specs === US_PASSPORT_SPECS) passes = false;
