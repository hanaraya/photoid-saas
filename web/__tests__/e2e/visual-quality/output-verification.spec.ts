...
const SCREENSHOT_DIR = path.join(
  __dirname,
  '../../../test-results/screenshots/output-verification'
);
...
test.describe('Output Verification - Download Functionality', () => {
  ...
  test('should download print sheet (4x6)', async ({ page }) => {
    ...
    const downloadSheetBtn = page
      .locator('button:has-text("Download Sheet")')
      .or(page.locator('button:has-text("Print Sheet")'));
    ...
    if (await downloadSheetBtn.isVisible({ timeout: 5000 })) {
      ...
      const matchesDimension = expectedDimensions.some(
        (d) =>
          (metadata.width === d.w && metadata.height === d.h) ||
          (Math.abs(metadata.width! - d.w) < 10 &&
            Math.abs(metadata.height! - d.h) < 10)
      );
  ...
  test('should contain multiple photos in print sheet', async ({ page }) => {
    ...
    if (await downloadSheetBtn.isVisible({ timeout: 5000 })) {
      ...
      console.log(
        `Sheet: ${sheetWidth}x${sheetHeight}, expected ~6 photos (2x2 on 4x6)`
      );
...
test.describe('Output Verification - Visual Quality', () => {
  ...
  test('should produce output with no visible compression artifacts', async ({
    page,
  }) => {
    const testPhoto = path.join(__dirname, '../../fixtures/test-portrait.jpg');
    ...
    const qualityAnalysis = await page.evaluate(() => {
      ...
      const maxBlocks =
        (Math.floor(height / blockSize) - 1) *
        (Math.floor(width / blockSize) - 1);
  ...
  test('should fill template correctly (no excessive margins)', async ({
    page,
  }) => {
    const testPhoto = path.join(__dirname, '../../fixtures/test-portrait.jpg');
...
test.describe('Output Verification - Print Layout', () => {
  ...
  test('should generate 4x6 print layout for US photos', async ({ page }) => {
    ...
    const hasSheetOption = await page
      .locator('text=Print Sheet')
      .or(page.locator('text=4×6'))
    ...
    if (hasSheetOption) {
      ...
      const downloadSheetBtn = page.locator(
        'button:has-text("Download Sheet")'
      );
...
test.describe('Output Verification - File Integrity', () => {
  ...
  test('should produce valid JPEG file', async ({ page }) => {
    ...
    const downloadSingleBtn = page.locator(
      'button:has-text("Download Single")'
    );
