...
export async function analyzeImageQuality(
  ...
  const checks = {
    ...
    resolution: checkResolution(metadata.width!, metadata.height!, expectedWidth, expectedHeight),
  };
  ...
  const criticalFailures = Object.values(checks).filter(c => c.severity === 'fail').length;
...
async function calculateMetrics(
  ...
  const backgroundWhiteness = calculateBackgroundWhiteness(pixels, width, height, channels);
...
function calculateLaplacianVariance(
  ...
  for (let y = 1; y < height - 1; y++) {
    for (let x = 1; x < width - 1; x++) {
      ...
      const left = (pixels[idxLeft] + pixels[idxLeft + 1] + pixels[idxLeft + 2]) / 3;
...
function calculateNoiseLevel(
  ...
  for (let i = 0; i < sampleSize; i++) {
    ...
    const localVar = localValues.reduce((sum, val) => sum + Math.pow(val - localMean, 2), 0) / 9;
...
function checkManipulationArtifacts(
  ...
  const score = Math.round((artifactScore * 0.5 + (hasBanding ? 0.3 : 1) * 0.5) * 100);
  
  return {
    passed: score >= 70,
    score,
    message: score >= 70 ? 'No obvious manipulation artifacts' : 'Possible manipulation artifacts detected',
...
function checkSkinToneNatural(colorDist: ColorDistribution): QualityCheck {
  ...
  const colorBalance = redMean >= greenMean * 0.9 && greenMean >= blueMean * 0.9;
  ...
  if (!saturationOk) {
    score -= 20;
    message = isOverSaturated ? 'Colors appear over-saturated' : 'Colors appear washed out';
  }
...
function checkBackgroundQuality(
  ...
  for (let y = info.height * 0.3; y < info.height * 0.7; y += 10) {
    ...
    const leftVal = (pixels[leftIdx] + pixels[leftIdx + 1] + pixels[leftIdx + 2]) / 3;
    
    // Check right edge  
    const rightIdx = (Math.floor(y) * info.width + (info.width - edgeWidth)) * info.channels;
    const rightVal = (pixels[rightIdx] + pixels[rightIdx + 1] + pixels[rightIdx + 2]) / 3;
    
    // If edge is brighter than expected (halo), reduce score
    if (leftVal > 250 || rightVal > 250) {
      // Check nearby pixel
      const nearLeftIdx = (Math.floor(y) * info.width + edgeWidth + 5) * info.channels;
      const nearLeftVal = (pixels[nearLeftIdx] + pixels[nearLeftIdx + 1] + pixels[nearLeftIdx + 2]) / 3;
  ...
  const finalScore = Math.round((isWhite ? 100 : backgroundWhiteness * 100) * 0.7 + haloScore * 0.3);
...
function checkHeadSizeRatio(
  ...
  const requirements: Record<string, { min: number; max: number }> = {
    ...
    india: { min: 0.50, max: 0.69 }, // Same as US
  };
...
function checkEyePosition(
  ...
  const requirements: Record<string, { min: number; max: number }> = {
    ...
    uk: { min: 0.64, max: 0.70 },
    eu: { min: 0.64, max: 0.70 },
  ...
  for (let y = info.height * 0.2; y < info.height * 0.6; y++) {
    ...
    for (let dy = 0; dy < bandHeight && y + dy < info.height; dy++) {
      for (let x = info.width * 0.25; x < info.width * 0.75; x++) {
        const idx = ((Math.floor(y) + dy) * info.width + Math.floor(x)) * info.channels;
  ...
  const isWithinRange = eyePositionFromBottom >= req.min && eyePositionFromBottom <= req.max;
  
  const score = isWithinRange 
    ? 100 
    : Math.max(0, 100 - Math.abs(eyePositionFromBottom - (req.min + req.max) / 2) * 200);
...
function checkLighting(averageBrightness: number, contrastRatio: number): QualityCheck {
  ...
  return {
    passed: score >= 70,
    score: Math.max(0, score),
    message: messages.length === 0 ? 'Lighting is even and appropriate' : messages.join('; '),
    severity: score >= 80 ? 'pass' : score >= 60 ? 'warn' : 'fail',
  };
}
...
function checkWhiteMargins(
  ...
  const marginTotal = marginSize * info.width * 2 + marginSize * info.height * 2;
  const totalWhite = topMarginWhite + bottomMarginWhite + leftMarginWhite + rightMarginWhite;
  const whiteRatio = totalWhite / marginTotal;
  
  // Some white is expected (background), but too much indicates excessive margins
  const hasExcessiveMargins = whiteRatio > 0.95 && (
...
function checkResolution(
  ...
  const score = hasGoodResolution && dimensionMatch ? 100 :
    hasGoodResolution ? 80 :
    minDimension >= 400 ? 60 : 30;
