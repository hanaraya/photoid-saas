...
export const COUNTRY_STANDARDS = {
  ...
  canada: { width: 591, height: 827, unit: 'mm' as const, specW: 50, specH: 70 },
  india: { width: 600, height: 600, unit: 'in' as const, specW: 2, specH: 2 },
  australia: { width: 413, height: 531, unit: 'mm' as const, specW: 35, specH: 45 },
 ...
 */
export async function uploadPhoto(page: Page, imagePath: string): Promise<void> {
  await page.locator('input[type="file"]').setInputFiles(imagePath);
  await page.locator('canvas').first().waitFor({ state: 'visible', timeout: 15000 });
  await page.locator('text=Face detected').or(page.locator('text=No face found')).first()
 ...
 */
export async function completeFlow(page: Page, imagePath: string): Promise<void> {
  await uploadPhoto(page, imagePath);
  await page.getByRole('button', { name: /Generate Printable Sheet/i }).click();
  await page.locator('text=Your Passport Photos').waitFor({ state: 'visible', timeout: 10000 });
}
 ...
 */
export async function selectCountryStandard(page: Page, standardId: string): Promise<void> {
 ...
 */
export async function waitForComplianceCheck(page: Page, checkName: string): Promise<void> {
  await page.locator(`text=${checkName}`).first().waitFor({ state: 'visible', timeout: 5000 });
}

/**
 * Get all compliance check statuses from the page
 */
export async function getComplianceStatuses(page: Page): Promise<Map<string, 'pass' | 'fail' | 'warn'>> {
  const statuses = new Map<string, 'pass' | 'fail' | 'warn'>();
  
  // Find all compliance check items
  const checks = await page.locator('[class*="compliance"], [data-testid*="check"]').all();
