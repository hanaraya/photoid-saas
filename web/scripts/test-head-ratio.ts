...
function calculateIdealRatio() {
  ...
  console.log('Difference:', ((idealRatio - 1.4) / 1.4 * 100).toFixed(1) + '%');
  
  // Now let's see the range
  const minBboxToHead = ANTHROPOMETRIC_DATA.blazeFaceBboxToFace.range.min * 
                        ANTHROPOMETRIC_DATA.faceToHeadRatio.range.min;
  const maxBboxToHead = ANTHROPOMETRIC_DATA.blazeFaceBboxToFace.range.max * 
                        ANTHROPOMETRIC_DATA.faceToHeadRatio.range.max;
  
  console.log('\nRatio range based on human variation:');
  console.log('  Min (large bbox, short forehead):', (1/maxBboxToHead).toFixed(2));
  console.log('  Max (small bbox, tall forehead):', (1/minBboxToHead).toFixed(2));
...
function simulateError() {
  ...
  for (const ratio of testRatios) {
    ...
    console.log(`  Ratio ${ratio.toFixed(2)}: ${adjusted.toFixed(1)}% ${status}`);
  }
}
